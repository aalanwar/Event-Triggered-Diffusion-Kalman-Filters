...
%clc; close all; %clear all;
close all;
keepvars = {'jj','reporterr_aggregated','reportmsg_aggregated'};
clearvars('-except', keepvars{:});
%FileName= strcat('cache\fourneig\ped04_fo_Th_',num2str(jj));

FileName= strcat('cache2ndRun\notcon\ped04_th_',num2str(jj));
load(FileName);
...
sentmsgsD
 ...
 reporterr_aggregated(jj+1,:) = reporterr;
 reportmsg_aggregated(jj+1,:) = reportmsg;
end

saveName= strcat('cache2ndRun\reports\Report_notcon_ped04');
